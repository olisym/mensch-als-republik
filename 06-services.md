...
- **S4 — Bewährtes adaptieren, nicht einbacken.** Wo etablierte Algorithmen ein Teilproblem lösen,
  ...
  **PoR/PoRep/PoSt-Klasse** für den vertagten Storage-Dienst. Das Atom bleibt algorithmus-agnostisch;
...
Dienst ankündigt. „Node" ist Betriebs-Vokabular (der Lebensraum, in dem Atome existieren, VISION §5),
...
- **`service-announce@1`, `timestamp@1`, `validation@1`** sind Profile über bestehenden Feldern
  (`I`, `J ∈ {claim-ref, object-hash}`, `v`, `N`, `t`, `h_prev`). Keiner besteht den `core`-Aufnahme-
